...
def test_missing_umis_and_unknown_libraries_leave_complete_counts_null(tmp_path):
    ...
    assert (alt["observed_labels"], alt["observed_cells"]) == (4, 4)
...
def test_one_label_in_two_read_groups_without_a_library_counts_twice(tmp_path):
    ...
    assert (alt["observed_labels"], alt["observed_cells"], alt["unknown_library_segments"]) == (2, 2, 2)
...
def test_reads_ending_at_an_insertion_are_labelled(tmp_path):
    ...
    assert (alt["segments"], alt["observed_labels"], alt["unresolved_segments"]) == (4, 4, 0)
...
def test_reads_the_collector_would_reject_are_unresolved_and_warned(tmp_path, caplog):
    ...
    assert alt["status_counts"]["metadata_unavailable"] == 3
    assert (alt["unresolved_segments"], alt["all_segments_labeled"], alt["complete_label_count"]) == (3, False, None)
...
def test_command_adds_cell_columns_and_requires_a_sample(tmp_path, capsys):
    ...
    assert row.num_alt_unknown_library_segments == 0
    ...
    assert event["cell_umi_alleles"]["alleles"]["alt"]["observed_cells"] == 2
...
def test_single_cell_ont_rna_reports_cells_behind_the_alt_allele_and_protein(tmp_path):
    ...
    event, = export["events"]
    assert event["cell_umi_alleles"]["alleles"]["alt"] == alt
